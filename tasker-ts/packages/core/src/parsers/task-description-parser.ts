...
export function syncMetadataToDescription(
  ...
  if (blocksIds?.length) parts.push(...blocksIds.map(id => `!${id}`));
  if (hasSubtaskIds?.length) parts.push(...hasSubtaskIds.map(id => `-^${id}`));
  if (blockedByIds?.length) parts.push(...blockedByIds.map(id => `-!${id}`));
  if (relatedIds?.length) parts.push(...relatedIds.map(id => `~${id}`));
  ...
  if (tags?.length) parts.push(...tags.map(t => `#${t}`));
